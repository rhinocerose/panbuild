...
%
\documentclass
	[$if(fontsize)$$fontsize$,$endif$$if(lang)$$babel-lang$,$endif$$if(papersize)$$papersize$paper,$endif$$for(classoption)$$classoption$$sep$,$endfor$]{$documentclass$}
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$if(fontfamily)$  \usepackage[$for(fontfamilyoptions)$$fontfamilyoptions$$sep$,$endfor$]{$fontfamily$}
$else$ 	\usepackage{lmodern}
$endif$
\usepackage{xcolor}
$if(linestretch)$
	\usepackage{setspace}
...
\ifnum 0\ifxetex 1\fi\ifluatex 1\fi=0 % if pdftex
	...
	$if(euro)$ \newcommand{\euro}{€}
	$endif$	
	$if(mainfont)$ \setmainfont[$for(mainfontoptions)$$mainfontoptions$$sep$,$endfor$]{$mainfont$}
	$endif$
	$if(sansfont)$ \setsansfont[$for(sansfontoptions)$$sansfontoptions$$sep$,$endfor$]{$sansfont$}
	$endif$
	$if(monofont)$ 	
		\setmonofont[Mapping=tex-ansi$if(monofontoptions)$,$for(monofontoptions)$$monofontoptions$$sep$,$endfor$$endif$]
		{$monofont$}
	$endif$
	$if(mathfont)$ \setmathfont(Digits,Latin,Greek)[$for(mathfontoptions)$$mathfontoptions$$sep$,$endfor$]{$mathfont$}
	$endif$
\fi
...
$if(verbatim-in-note)$ \VerbatimFootnotes % allows verbatim text in footnotes
$endif$

$if(geometry)$ \usepackage[$for(geometry)$$geometry$$sep$,$endfor$]{geometry}
$endif$
...
$if(lhs)$ \lstnewenvironment{code}{\lstset{language=Haskell,basicstyle=\small\ttfamily}}{}
$endif$
...
$if(documentation)$
	\pagestyle{fancy}
		...
		\fancyhead[R]{\fontsize{10pt}{11pt}\selectfont
			\textbf{\authorname{}} \color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
				$endif$}\& \color{black}\textbf{\authornameb{}} } 
		\else
			\fancyhead[R]{\fontsize{10pt}{11pt} \textbf{$author$}  \color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
					$endif$} $studentnumber$ }
		\fi
		\fancyhead[L]{\fontsize{10pt}{11pt}\selectfont
		\textbf{$classnum$:}  \color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
			$endif$}$title$}
		\fancyfoot[C]{\bfseries\fontsize{10pt}{12pt}\color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
				$endif$}\selectfont\thepage}
...
$else$
	...
	\ifdefined\authornameb
		...
		\textbf{\authorname{}} \color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
			$endif$}\& \color{black}\textbf{\authornameb{}} } 
	\else
		\fancyhead[R]{\fontsize{10pt}{11pt}\textsf{\textbf{$author$}} \textsf{\color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
					$endif$}$studentnumber$}}
	\fi
	\fancyhead[L]{\sffamily\fontsize{10pt}{11pt}\selectfont
		\textbf{$classnum$:}  \color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
			$endif$}$title$}
	\fancyfoot[C]{\sffamily\bfseries\fontsize{10pt}{12pt}\color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
			$endif$}\selectfont\thepage}
	\renewcommand{\headrulewidth}{0.5pt}
	\renewcommand{\footrulewidth}{0.5pt}
	\fancypagestyle{plain}{
...
$if(title)$
	$if(documentation)$
		 \title{
		 	...
		 	\begin{flushright}
		 		\vspace{-7mm} 
		 		\fontsize{22pt}{22pt} \color{black} \textbf{$title$} \\  \vspace{0.3em}
		 		$if(subtitle)$ 
	...
	$if(homework)$
		\title{	
			...
			\begin{flushright} 
				\vspace{-3em}
				\large \textbf{\textsf{$author$}} \\ \vspace{-0.03em}
				\normalsize \textbf{\textsf{\color{$maincolor$}$studentnum$}} \\  \vspace{-0.05em} 
				\textsf{\today} \\ \vspace{1.15em}
			\end{flushright}
			\vspace{-6.0em}
			\begin{flushleft} 
				\Huge \textbf{\textsf{\color{$maincolor$}$classnum$} }\\ % Assignment title 
				\vspace{0.07em}
				\Large \textbf{\textsf{\color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
							$endif$}$title$}} \\
				\vspace{-0.3em}
			\end{flushleft}
	...
	$if(labreport)$
		...
		\begin{flushright}
			...
			\vspace*{0em}			
			\noindent
			\hspace{-0.40em}\YUGE 	\bfseries	\color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
				$endif$} \textbf{$classnum$} \\*[-3.9mm]
		...
		\begin{flushleft}
			...
			$if(subtitle)$	
				\hspace{-0.65em} \subtitle	\color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
					$endif$} \text{ $subtitle$} 	\\*[5em]
			$endif$
			$if(author)$
				\Huge	\color{black} \text{$author$}		\\*[-0.05em]
			$endif$
			$if(studentnum)$
				\LARGE	\color{$if(grayscale)$ $othercolorbw$ $else$ $othercolor$
					$endif$} \text{$studentnum$}	\\*[4em]
			$endif$
			$if(institute)$ 
...
%		\sffamily
		...
		$if(toc)$	{
			$if(colorlinks)$ \hypersetup{linkcolor=$if(toccolor)$$toccolor$$else$black$endif$}
			$endif$
			...
			$endif$
			$if(lot)$ \listoftables	\vspace{5mm}
			$endif$
			\cleardoublepage
	...
	$if(documentation)$
		\maketitle
		$if(abstract)$ \begin{abstract} $abstract$ \end{abstract}
		$endif$
	$endif$
	$if(homework)$ \maketitle \vspace{3mm}
	$endif$		
$endif$
...
$if(natbib)$
	$if(bibliography)$
		$if(biblio-title)$
			$if(book-class)$ \renewcommand\bibname{$biblio-title$}
			$else$    		 \renewcommand\refname{$biblio-title$}
			$endif$
		$endif$
...
$if(biblatex)$ \printbibliography
	$if(biblio-title)$ [title=$biblio-title$]
	$endif$
$endif$
